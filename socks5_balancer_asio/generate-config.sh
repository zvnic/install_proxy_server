$()$(
    ...
    for server in "${servers[@]}"; do
        ...
        # Удаляем возможные лишние двоеточия или некорректные символы из port
        port=$(echo "$port" | tr -d ':')

        # Проверяем, что port является числом
        if ! [[ "$port" =~ ^[0-9]+$ ]]; then
            echo "Ошибка: Порт должен быть числом: $port в $server"
            exit 1
        fi
...
EOF
    ...
    if command -v jq >/dev/null 2>&1; then
        jq . config.json >/dev/null 2>&1 || {
            echo "Ошибка: Сгенерированный config.json не является валидным JSON"
            exit 1
        }
    fi
)$()
